...
def penultimate_weight_regularization():
    weights, _ = model.layers[-1].weights # Does not need bias
    reject_class_weights = weights[:, -1]
    max_class_weights = tf.reduce_max(weights, axis=1)
    return tf.reduce_sum(tf.math.square(max_class_weights - reject_class_weights))
...
def train_step(inputs):
    ...
    with tf.GradientTape() as tape:
        ...
        regularization_loss = lambda_ * penultimate_weight_regularization()
