...
student_outputs = [layer.output for layer in trained_layers] + [model.output]
teacher_outputs = [layer.output for layer in teacher_layers]
student_all_layer_model = tf.keras.Model(inputs=model.input, outputs=student_outputs, name='student_all_layer_model')
teacher_internal_output_model = tf.keras.Model(inputs=teacher_model.input, outputs=teacher_outputs, name='teacher_internal_output_model')

# Loss Functions
def cross_entropy(y_true, y_pred):
    return tf.keras.losses.categorical_crossentropy(y_true, y_pred)
def internal_output_diff(teacher_internal_output, student_internal_output):
    loss = 0.
    for student, teacher in zip(student_internal_output, teacher_internal_output):
        loss += tf.reduce_mean(tf.math.square(tf.reshape(student, [-1]) - tf.reshape(teacher, [-1])))
    return loss
...
def train_step(inputs):
    ...
    with tf.GradientTape() as tape:
        outputs = student_all_layer_model(images, training=True)
        preds = outputs[-1]
        internal_outputs = outputs[:-1]
        normal_loss = compute_loss(cross_entropy, labels, preds)
        regularization_loss = lambda_ * internal_output_diff(teacher_internal_output_model(images), internal_outputs)
...
def valid_step(inputs):
    images, labels = inputs
    outputs = student_all_layer_model(images, training=False)
    preds = outputs[-1]
    internal_outputs = outputs[:-1]
    normal_loss = cross_entropy(labels, preds)
    regularization_loss = lambda_ * internal_output_diff(internal_outputs, teacher_internal_output_model(images))
    loss = normal_loss + regularization_loss
